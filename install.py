...
import sys
import os
from mypylib import Dict, MyPyClass, read_config_from_file
from utils import (
    import_modules,
    get_modules_names,
    get_module_by_name,
...
def calculate_space_to_provide(answers):
    ...
    total_space, used_space, free_space = get_disk_space(
        storage_path, decimal_size=3, round_size=0
    )
...
def validate_utils(answers, current):
    if len(current) == 0:
        raise inquirer.errors.ValidationError(
            "", reason="Выберите хотя бы один модуль (нажмите Пробел)!"
        )
    return True
...
def create_questions():
    questions = [
        inquirer.Checkbox(
            ...
            validate=validate_utils,  # <-- Мы добавили эту строку
        ),
        inquirer.Text(
            name="storage_path",
