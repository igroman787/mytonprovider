...
class Module:
    ...
    def get_daily_statistics_data(self, comparing_days):
        if self.local.db.daily_statistics == None:
            raise Exception(
                "get_daily_statistics_data error: local.db.daily_statistics is None"
            )
        # end if
        ...
        data.recv = convert_to_required_decimal(
            zero_day.bytes_recv - comparing_day.bytes_recv, decimal_size=3, round_size=2
        )
        data.sent = convert_to_required_decimal(
            zero_day.bytes_sent - comparing_day.bytes_sent, decimal_size=3, round_size=2
        )
    ...
    def save_disk_statistics(self):
        ...
        for name in disks:
            ...
            disk_load5, diskLoadPercent5, iops5 = self.calculate_disk_statistics(
                zerodata, buff5, name
            )
            disk_load15, diskLoadPercent15, iops15 = self.calculate_disk_statistics(
    ...
    def calculate_disk_statistics(self, zerodata, data, name):
        ...
        disk_load = convert_to_required_decimal(
            disk_read + disk_write, decimal_size=2, round_size=2
        )
    ...
    def save_network_statistics(self):
        ...
        net_recv_avg1, net_sent_avg1, networkLoadAvg1, ppsAvg1 = (
            self.calculate_network_statistics(zerodata, buff1)
        )
        net_recv_avg5, net_sent_avg5, networkLoadAvg5, ppsAvg5 = (
            self.calculate_network_statistics(zerodata, buff5)
        )
        net_recv_avg15, net_sent_avg15, networkLoadAvg15, ppsAvg15 = (
            self.calculate_network_statistics(zerodata, buff15)
        )
    ...
    def calculate_network_statistics(self, zerodata, data):
        ...
        net_recv_avg = convert_to_required_decimal(
            bits_recv_avg, decimal_size=2, round_size=2
        )
        net_sent_avg = convert_to_required_decimal(
            bits_sent_avg, decimal_size=2, round_size=2
        )
        net_load_avg = convert_to_required_decimal(
            bits_recv_avg + bits_sent_avg, decimal_size=2, round_size=2
        )
