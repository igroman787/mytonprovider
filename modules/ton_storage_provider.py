...
class Module:
    ...
    async def register(self, args):
        ...
        if (
            self.local.db.ton_storage.provider.is_already_registered
    ...
    async def import_wallet(self, args):
        try:
            ...
            color_print(
                "{red}Bad args. Usage:{endc} import_wallet <wallet-private-key>"
            )
            return
    ...
    def do_import_wallet(self, privkey):
        ...
        provider_config = self.get_provider_config()
        provider_config.ProviderKey = base64.b64encode(provider_key_bytes).decode(
            "utf-8"
        )
    ...
    def print_profit(self):
        ...
        text = self.local.translate("provider_profit").format(
            real_profit_text, max_profit_text
        )
        print(text)
    ...
    def print_provider_space(self):
        ...
        text = self.local.translate("provider_space").format(
            used_provider_space_text, total_provider_space_text
        )
        print(text)
    ...
    def get_used_provider_space(self, decimal_size, round_size):
        ...
        used_provider_space = ton_storage_module.get_bags_size(
            api_data, decimal_size, round_size
        )
    ...
    def install(self, install_args, install_answers):
        ...
        provider_config.MinRatePerMBDay = self.calculate_MinRatePerMBDay(
